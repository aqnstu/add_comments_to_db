'''
    Скрипт для добавления комментариев в БД ИАС.
'''
import datetime
import glob
...
def get_all_files_from_dir(dir, type):
...
def update_comments_in_db(db_name='ias'):
    ...
    csv_file_names = [path_name.split('.')[0] for path_name in list(map(os.path.basename, csv_file_paths))]
    ...
    with engine.connect() as conn:
        with conn.begin():
            for schema_name in csv_file_names:
                ...
                existing_full_table_names.extend([schema_name + '.' + row[0] for row in result])
            ...
            for table_name, comment in zip(columns[0], columns[1]):
                try:
                    with conn.begin():
                        ...
                        engine.execution_options(autocommit=True).execute(f"COMMENT ON TABLE {full_table_name} is \'{comment}\'")
    ...
    log_table = t.tabulate(
        log,
        headers=['schema', 'table', 'comment', 'date_added'],
        tablefmt='psql'
    )
    ...
    with open(os.path.join('log', f"{db_name}_{current_time}.txt"), 'w+') as f:
        f.write("Таблицы, указанные в файлах директории 'csv', которые не удалось обновить:\n")
        f.write(log_table)
        f.write("\n\n")
    ...
    remaining_full_table_names = list(set(existing_full_table_names) - set(processed_full_table_names))
    
    remaining_full_table_names_table = t.tabulate(
        [table_name.split('.') for table_name in sorted([record + '.' + current_time for record in remaining_full_table_names])],
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        try:
            update_comments_in_db(sys.argv[1])
